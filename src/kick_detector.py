...
class BLESource:
    ...
    def __init__(self, sim_left=False, sim_right=False):
        import asyncio
        ...
        self._sim_left  = SimulatedSensor(kick_rate=0.6, seed=42) if sim_left else None
        self._sim_right = SimulatedSensor(kick_rate=0.6, seed=99) if sim_right else None
        ...
        self._loop.run_until_complete(self._connect(sim_left=sim_left, sim_right=sim_right))
    ...
    async def _connect(self, sim_left=False, sim_right=False):
        need_left  = not sim_left
        need_right = not sim_right
        print("Scanning for XIAO sensors (20s)...")
        devices = await self._BleakScanner.discover(timeout=20.0)
        ...
        if need_left and not left_dev:
            raise RuntimeError("XIAO-LEFT not found — make sure it's powered on and advertising")
        if need_right and not right_dev:
            raise RuntimeError("XIAO-RIGHT not found — make sure it's powered on and advertising")
        ...
        if right_dev:
            ...
            print("XIAO-RIGHT simulated ✅")

    def read(self) -> tuple[tuple, tuple]:
        self._loop.run_until_complete(self._asyncio.sleep(0.01))
        left  = self._sim_left.read()  if self._sim_left  else self._left_xyz
        right = self._sim_right.read() if self._sim_right else self._right_xyz
        return left, right
...
class LivePlotter:
    ...
    def __init__(self, threshold: float):
        ...
        self._left_kicks:  list[tuple[int, bool]] = []   # (sample_index, is_selective)
        self._right_kicks: list[tuple[int, bool]] = []
    ...
    def push(self, l_mag: float, r_mag: float,
             ...
             l_selective: bool = False, r_selective: bool = False):
        with self._lock:
            self._left_mag.append(l_mag)
            self._right_mag.append(r_mag)
            if l_kicked:
                self._left_kicks.append((self._sample_count % PLOT_SAMPLES, l_selective))
            if r_kicked:
                self._right_kicks.append((self._sample_count % PLOT_SAMPLES, r_selective))
    ...
    def _animate(self, _frame):
        with self._lock:
            ...
            for ax, mag, kicks, color in [
                ...
                ax.axhline(self._threshold, color="gray", linestyle="--",
                           linewidth=0.8, label=f"Threshold ({self._threshold}g)")
                for k, sel in kicks:
                    ax.axvline(k, color="green" if sel else "red", alpha=0.6, linewidth=1.2)
                ax.set_ylim(0, 4)
                ax.grid(True, alpha=0.3)
                ax.legend(loc="upper right", fontsize=8)
        ...
        self._left_kicks  = [(k, s) for k, s in self._left_kicks  if k >= 0][-20:]
        self._right_kicks = [(k, s) for k, s in self._right_kicks if k >= 0][-20:]
...
def run(source, threshold: float, no_plot: bool):
    ...
    try:
        while True:
            ...
            if plotter:
                l_sel = next((s for leg, s, _ in events if leg == "left"),  False)
                r_sel = next((s for leg, s, _ in events if leg == "right"), False)
                plotter.push(l_mag, r_mag, l_kicked, r_kicked, l_sel, r_sel)
                plotter.flush()
            else:
...
def main():
    ...
    if args.ble:
        ...
        source = BLESource(sim_left=args.sim_left, sim_right=args.sim_right)
